...
class CategoryQuerySet(models.QuerySet):
    ...
    def user_accessible(self, user):
        ...
        pks = []
        for category in self.filter(is_listed=True, group_limit__isnull=False):
            if category.group_limit.filter(pk=user_group.pk).exists():
                pks.append(category.pk)

        universally_accessible_categories = self.filter(group_limit__isnull=True)
